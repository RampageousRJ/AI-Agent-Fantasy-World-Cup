...
- Risk Play was chosen from the official claim catalog, or skipped if uncertain

Do not mention a formation unless the Fantasy XI position count gate has passed.
Do not mention player names, team strength, likely starters, odds, stakes, labels, or hidden analysis unless those facts are present in official files.
Do not include Markdown, tables, code blocks, local paths, credentials, or extra JSON.

Safe default:

```text
Selected official eligible player IDs from the current board, built the Fantasy XI by position bucket, and checked the required counts before final output. The Risk Play field uses only official claim fields from the current catalog.
```
